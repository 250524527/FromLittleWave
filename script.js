document.addEventListener('DOMContentLoaded', () => {
    ...
    // 모달 안의 '자료실로 복귀' 버튼을 눌렀을 때 이동하는 함수
    function goToArchive() {
        // 그냥 이동하면 심심하니 아까 만든 기밀해제 연출을 거쳐가게 해도 좋고, 즉시 가도 좋네!
        window.location.href = "archive.html";
    }
    ...
    function checkAnswer() {
        ...
        const correctAnswer = "초동55"; // 자네가 정한 정답 번호

        if (userAnswer === correctAnswer) {
            // 1. 다음 단계(code2)를 열기 위해 키를 저장하네!
            localStorage.setItem('unlocked_code1', 'true');
            
            // 2. 꽁꽁 숨겨둔 이미지 모달을 짠! 하고 보여줌
            const successModal = document.getElementById('successModal');
            if (successModal) {
                successModal.style.display = 'flex';
            }
        } else {
            alert("❌ [ACCESS DENIED] 잘못된 주파수 코드입니다.");
        }
    }
